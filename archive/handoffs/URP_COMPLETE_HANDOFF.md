...
> Historical consolidated handoff retained as source provenance. For the
> implemented product, use `README.md`, `docs/MIGRATION_NOTES.md`, and
> `TEST_RESULTS.md`.
...
Current platform-ready addendum: the canonical monorepo now includes `docs/13_PLATFORM_READINESS.md`, `docs/WHITE_PAPER.md`, `examples/USE_CASES.md`, and `examples/live/run_live_examples.py`. Run `PYTHONPATH=python python3 -m urp.cli platform matrix` for local, Kubernetes, AWS, Azure, GCP, on-prem, edge, OpenAI-compatible provider, and CI/CD contract readiness. Run `PYTHONPATH=python python3 examples/live/run_live_examples.py --reset` for live JSON evidence covering exact object restore, byte-range reads, legal-hold delete denial, AI exact cache hits, mock lakehouse adapter execution, dashboard sections, savings metrics, and ledger-chain verification. The current top-level `README.md`, `TEST_RESULTS.md`, `package_manifest.json`, and `codex/ACCEPTANCE_CHECKLIST.md` are the most current package entry points.
