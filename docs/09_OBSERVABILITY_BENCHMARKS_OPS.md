...
The local reference implementation writes redacted JSONL operational logs to
the state directory and exposes them through:

```bash
PYTHONPATH=python python3 -m urp.cli logs query --tenant acme --event-type manifest.written
curl -X POST http://127.0.0.1:8080/v1/logs/query \
  -H 'content-type: application/json' \
  -d '{"tenant":"acme","event_types":["manifest.written"],"limit":20}'
```

Prompt, payload, body, content, token, and secret fields are redacted before
persistence. Use the ledger for governance or audit reconstruction.

The local manifest explorer is available through:

```bash
PYTHONPATH=python python3 -m urp.cli manifest explore --tenant acme
curl http://127.0.0.1:8080/v1/manifests/explore?tenant=acme
```

It returns redacted manifest rows, byte totals, verifier status, cache result,
and aggregate counts by kind, contract, and state.

## Dashboards

The local reference implementation exposes a dashboard payload through:

```bash
PYTHONPATH=python python3 -m urp.cli report dashboard
curl http://127.0.0.1:8080/v1/reports/dashboard
```

The payload has `executive`, `platform`, `ai`, `data`, and `security` sections. Byte savings, cache hit rate, token counts, verifier failures, policy denials, fallback events, manifest counts, and ledger event counts come from manifests and ledger rows. Latency overhead is reported as p95 over recorded local overhead spans; when no matching spans exist, the value is `0.0` with an explanatory note rather than a fabricated estimate.
...
```bash
PYTHONPATH=python python3 -m urp.cli benchmark run --suite prompt-cache-v1
```

### Local load suite

`load-local-v1` is a bounded local load smoke suite. It reports the dimensions
required by the implementation blueprint without external services:

- object ingest throughput;
- rehydration p95 latency;
- AI gateway p95 latency;
- exact cache index write and lookup rate;
- manifest store write rate.

Local command:

```bash
PYTHONPATH=python python3 -m urp.cli benchmark run --suite load-local-v1
```
