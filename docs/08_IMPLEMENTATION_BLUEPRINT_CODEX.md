...
> Historical build specification. The canonical implementation has landed at
> the repository root; use `README.md`, `docs/MIGRATION_NOTES.md`, and
> `TEST_RESULTS.md` for current behavior and verification status.
...
Local reference status: the Python runtime exposes Prometheus metrics, JSONL
trace spans, redacted JSONL operational logs, `urp manifest explore`, `urp logs
query`, and `urp ledger query`, with REST mirrors for dashboards, traces, logs,
ledger streaming, and manifest exploration.
...
Local reference status: active policy bundles can be reloaded through
`urp policy reload --name <bundle>`, `/v1/policies/bundles/{name}/reload`, and
SDK helpers. Reload validates the active bundle and emits
