...
The local-ideal runtime supports opaque API keys configured through
`URP_LOCAL_API_KEY` and `URP_API_KEYS_JSON`, tenant-bound RBAC, signed approvals,
and role-aware redaction. Production adapters should additionally support:
...
The implemented local roles are `admin`, `operator`, `developer`, `gateway`,
and `viewer`. Only operator/admin-style permissions expose sensitive manifest
locations; viewer and gateway responses are recursively redacted. Actor fields
for policies, plugins, approvals, deletion, KMS, and administrative events come
